...
PAVEMENT_CLASS_MAPPING_EXCEL = {
    "Paved": 1,  # "Sealed",
    "Unpaved": 2,  # "Unsealed",
}
TECHNICAL_CLASS_MAPPING_EXCEL = {
    "R3": 6,
    "R5": 7,
}
ROAD_STATUS_MAPPING_EXCEL = {
    "Planned": 4,
    "Pending": 3,
    "Ongoing": 2,
    "Complete": 1,
    ...
    for file_name, road_type, populate in sources:
        ...
        for feature in shp_file[0]:
            ...
            with reversion.create_revision():
                road.save()
                reversion.set_comment("Imported - {} - feature id({})".format(file_name, feature.fid))
...
def populate_road_rrpmis(road, feature):
    ...
    if surface_condition and surface_condition != '0' and surface_condition != 'Unlined':
        road.surface_condition = SURFACE_COND_MAPPING_RRMPIS[
            feature.get('pvment_con')
        ]
    ...
    if maintenance_needs and maintenance_needs != '0':
        ...
        road.maintenance_need = MaintenanceNeed.objects.get(
            code=maint_code
        )
...
def import_csv():
    ...
    zumalai_suai = Road.objects.get(road_name='Zumalai (Junction A12) - Suai (Junction C21)')
...
def populate_from_csv(road, row):
    ...
    simple_assignments = dict(road_name='Road name', carriageway_width='Carriageway width', funding_source='Funding source', project_name='Project name')
