...
export function toDms(latLon: [number, number]) {
    if (!latLon) {
        return "";
    }
    ...
    return Math.abs(yDms[0]) + "\u00b0" + yDms[1] + "'" + yDms[2] + '"' + NorS + " "
        + Math.abs(xDms[0]) + "\u00b0" + xDms[1] + "'" + xDms[2] + '"' + EorW;
}

export function toUtm(latLon: [number, number]) {
    return latLon
